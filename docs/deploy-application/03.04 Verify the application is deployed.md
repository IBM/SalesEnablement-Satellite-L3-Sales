...
??? info "If the site can't be reached..."
    Some users have encountered an issue when attempting to access the OpenShift web console of the cluster running in AWS.  The root cause appears to be the user's Internet Service Provider (ISP) using IPv6. The issue was resolved by adding or changing the DNS server on their local machine to 1.1.1.1. If the connection to the web console times out or you get an error message saying "The site can't be reached." you may want to try this.
