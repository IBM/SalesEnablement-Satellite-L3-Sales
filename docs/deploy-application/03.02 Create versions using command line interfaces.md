...
Now that you are familiar with the CLIs for IBM Cloud Satellite, create the other **versions** for our demonstration.  The CLI syntax to create a version is:
...
- Create the remaining 5 versions required for the sample application.
...
Now that the script has executed, return to the IBM Cloud Portal and verify all 6 versions now exist. You should also explore the YAML files that are used by each subscription. You can do this using the IBM Shell command and commands like **vi** or **cat**, or return to your **Satellite configuration** in the IBM Cloud Portal and look and view the **versions** that have been created. You should see a total of seven versions. The six that you created:
